"""Thin synchronous client for the Facebook Reels & Video Scraper Apify Actor."""
...
from .exceptions import ActorRunError, ActorTimeoutError, AuthenticationError, FacebookReelsVideoScraperError

ACTOR_ID = "apivault_labs~facebook-reels-video-scraper"
APIFY_API_BASE = "https://api.apify.com/v2"
TERMINAL_FAIL = {"FAILED", "TIMED-OUT", "ABORTED"}


class FacebookReelsVideoScraperClient:
    ...
    def __init__(self, api_token: str | None = None, timeout: int = 600,
        ...
        self.session.headers.update({
            ...
            "User-Agent": "facebook-reels-video-scraper-python/0.1.0",
        })
    ...
    def _start(self, payload: dict[str, Any], timeout_secs: int) -> str:
        try:
            ...
            raise FacebookReelsVideoScraperError(f"Could not start Actor run: {exc}") from exc
    ...
    def _wait(self, run_id: str) -> dict[str, Any]:
        ...
        while True:
            try:
                ...
                raise FacebookReelsVideoScraperError(f"Could not poll Actor run: {exc}") from exc
            if response.status_code >= 400:
    ...
    def _dataset(self, dataset_id: str) -> list[dict[str, Any]]:
        try:
            ...
            raise FacebookReelsVideoScraperError(f"Could not download Dataset: {exc}") from exc
        if response.status_code >= 400:
